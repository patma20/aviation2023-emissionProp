...
\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    \node [element] (lpc) {Low Pressure Compressor};
    \node [element, right=4em of lpc ] (injector) {Water Injector};
    \node [element, right=4em of injector ] (hpc) {High Pressure Compressor};
    ...
    \node [element, right=4em of lpt] (extractor) {Water Extractor};
    \node [element, right=4em of extractor] (corenozzle) {Core Nozzle};
    \node [element, below=8em of burner] (vrl) {Vapor Recovery Loop};
    % \node [element, below=12em of burner] (bypduct) {Bypass Duct};
    % \node [element, below=12em of corenozzle] (bypnozzle) {Bypass Nozzle};
    ...
    % \node [station_label, below=6em of fan.west] (1) {1};
    ...
    % \node [station_label, below=6em of bypnozzle.west] (17) {17};
    ...
    \path [connection] (lpc) -- (injector);
    \path [connection] (injector) -- (hpc);
    \path [connection] (hpc) -- (burner);
    \path [connection] (burner) -- (hpt);
    \path [connection] (hpt) -- (lpt);
    \path [connection] (lpt) -- (extractor);
    \path [connection] (extractor) -- (corenozzle);
    \path [connection] (extractor) |- (vrl);
    \path [connection] (vrl) -| (injector);

    % Draw station label connections
    % \path [line] (inlet.west) -- (1.north);
    % \path [line] (fan.west) -- (1.north);
    ...
    % \path [line] (bypnozzle.west) -- (17.north);
