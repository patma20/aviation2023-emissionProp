...
% To utilize water engine section in front of the combustor the N+3 engine model had to be modified to include $H_2O$ as a species.
...
Therefore, we will use TSEC at the cruise condition to compare the two fuels but will use $\dot{m}_{\rm{fuel,CRZ}}$ as the objective function in the optimzation problem.
...
Only the water recovery fraction at CRZ is a design variable since we are optimizing $\dot{m}_{\rm{fuel,CRZ}} $.
...
Our fuel performance metric is TSEC but minimizing $\dot{m}_{\rm{fuel}}$ is a more relavent problem since we will have known thrust constraints and want to compare fuel economy.
Therefore, the fuel flow rate at CRZ, $\dot{m}_{\rm{fuel,CRZ}}$, was set as the objective function.
