...
In section \ref{sec:optprob} the implementation of the multi-point optimization problem is discussed. Finally, we present the results from the optimization problem in section \ref{sec:results}.
...
In this work we use the NASA advanced technology UHB geared turbofan engine cycle, referred to as the "N+3" engine \cite{Jones2017a}.
...
In pyCycle, a direct solver is used for linear equations and a Newton solver is used for nonlinear equations which uilizes the analytic partial derivatives provided by each OpenMDAO component.
...
Therefore, the N+3 enigne model computes the engine performance at 4 different operating conditions in order to resolve the complexities between each operating condition on the final design.
...
In the optimization problem described later, we set the mass flow rate of fuel, $\dot{m}_{fuel}$, as the objective fuction.
...
Fortunatly, pyCycle comes ready to use hydrogen with cateloged exhaust products for hydrogen fuel.
